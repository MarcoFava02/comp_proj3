...
// Definition of count_inside
// This function counts how many particles are still inside the trap, i.e. particles with norm(r) < d
double PenningTrap::count_inside(){
    double counter = 0.;
    for(int i=0; i<Particle_.size(); i++){
        if(arma::norm(Particle_[i].r_) < d_){
            counter += 1;
        }
    }
    return counter;
...
// Definition of add_particle_random
// This function add a Calcium particle with randomly initial position and velocity.
void PenningTrap::add_particle_random(double q, double m){
  arma::arma_rng::set_seed_random();                    // Set the seed for Armadillo’s random number generator
  ...
  //Create the particle
  Particle part_rand(q,m,r_rand,v_rand);
  //Add it to the vector
  Particle_.push_back(part_rand);

}
#endif /* SourceFile9_hpp */