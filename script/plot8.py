...
if len(sys.argv) != 5:
    ...
    print(f"Usage: {script_name} <\"true\"/\"false\"/\"both\" to have interaction or not or both> <integer number of steps> <fe=Forward-Euler, rk=RK4, an=analytical solution> <letter of a problem \"a\" to \"d\">")
    sys.exit(1)
...
method = sys.argv[3]                # method used to do get the data: "fe"=Forward Fulero, "rk"=Runge Kutta, "an"=Analytical Solution
...
if (do_interation.lower() == "true") or (do_interation.lower() == "both"):
    ...
    # filename_inter = "data_inter4000.txt"
...
if (do_interation.lower() == "false") or (do_interation.lower() == "both"):
    ...
    # filename_nointer = "data_nointer4000.txt"
...
if prob == "a":
    ...
    m = 40.078
    d = 500
    omegaz = np.sqrt(2*q*V/m/d**2)
    T = 2*np.pi/omegaz
    print(T)
    # Find the first max, after the one in t=0, to mark it in the plot
    start_index = 700
    ...
    # Prepare the plot
    plt.plot(t_n,z_n1,'-',lw=lin,label=f"Particle $a$, T={T:.2f}")
    plt.scatter(t_max,z_max,label=f'max = ({t_max:.2f},{z_max:.0f})')
    plt.xlabel('t [s]',fontsize=font_axl)
    plt.ylabel('z [$\mu m$]',fontsize=font_axl)
    # plt.title("simulation of the z position of a single particle",fontsize=font_ttl) # plt.title("Simulation of a single particle, $z(t)=z_0cos(\omega_zt)+\\frac{v_z}{\omega_z}sin(\omega_zt)$",fontsize=font_ttl)
...
if prob == "b":
    ...
    plt.plot(x_i1,y_i1,lw=lin,label='Particle $a$')
    plt.plot(x_i2,y_i2,lw=lin,label='Particle $b$')
    plt.xlabel('x [$\mu m$]',fontsize=font_axl)
    plt.ylabel('y [$\mu m$]',fontsize=font_axl)
    # plt.title('Simulation of two particles that interact',fontsize=font_ttl)
    ...
    plt.plot(x_n1,y_n1,lw=lin,label='Particle $a$')
    plt.plot(x_n2,y_n2,lw=lin,label='Particle $b$')
    plt.plot(x_big,y_big,'g',lw=1)
    plt.plot(x_small,y_small,'g',lw=1)
    plt.xlabel('x [$\mu m$]',fontsize=font_axl)
    plt.ylabel('y [$\mu m$]',fontsize=font_axl)
    # plt.title('Simulation of two particles that don\'t interact',fontsize=font_ttl)
...
if prob == "c":
    ...
    plt.plot(x_i1,vx_i1,lw=lin,label='Particle $a$')
    plt.plot(x_i2,vx_i2,lw=lin,label='Particle $b$')
    plt.xlabel('x [$\mu m$]',fontsize=font_axl)
    plt.ylabel('$v_x$ [$\mu m/\mu s$]',fontsize=font_axl)
    # plt.title('Two particles, interaction, phase diagram (x,$v_x$)',fontsize=font_ttl)
    ...
    plt.plot(x_n1,vx_n1,lw=lin,label='Particle $a$')
    plt.plot(x_n2,vx_n2,lw=lin,label='Particle $b$')
    plt.plot(x,vx_max,'-k',lw=lin,label='$v_x^{max}=31.72m/s$')
    plt.xlabel('x [$\mu m$]',fontsize=font_axl)
    plt.ylabel('$v_x$ [$\mu m/\mu s$]',fontsize=font_axl)
    # plt.title('Two particles, no interaction, phase diagram (x,$v_x$)',fontsize=font_ttl)
    ...
    plt.plot(z_i1,vz_i1,lw=lin,label='Particle $a$')
    plt.plot(z_i2,vz_i2,lw=lin,label='Particle $b$')
    plt.xlabel('y [$\mu m$]',fontsize=font_axl)
    plt.ylabel('$v_z$ [$\mu m/\mu s$]',fontsize=font_axl)
    # plt.title('Two particles, interaction, phase diagram (z,$v_z$)',fontsize=font_ttl)
    ...
    plt.plot(z_n1,vz_n1,lw=lin,label='Particle $a$')
    plt.plot(z_n2,vz_n2,lw=lin,label='Particle $b$')
    plt.xlabel('y [$\mu m$]',fontsize=font_axl)
    plt.ylabel('$v_z$ [$\mu m/\mu s$]',fontsize=font_axl)
    # plt.title('Two particles, no interaction, phase diagram (z,$v_z$)',fontsize=font_ttl)
...
if prob == "d":
    ...
    #create the 3D plot
    ax.plot(x_n1,y_n1,z_n1,lw=lin,label='Particle $a$')
    ax.plot(x_n2,y_n2,z_n2,lw=lin,label='Particle $b$')
    ...
    ax.tick_params(axis='y', labelsize=font_ax)
    ax.legend(fontsize=font_lg,bbox_to_anchor=(0.6,0.8))#loc="upper right",
    plt.show()
    ...
    #create the 3D plot
    ax.plot(x_i1,y_i1,z_i1,lw=lin,label='Particle $a$')
    ax.plot(x_i2,y_i2,z_i2,lw=lin,label='Particle $b$')
    ax.set_xlabel('x [$\mu m$]',fontsize=font_axl)
    ax.set_ylabel('y [$\mu m$]',fontsize=font_axl)
    ax.set_zlabel('y [$\mu m$]',fontsize=font_axl)
