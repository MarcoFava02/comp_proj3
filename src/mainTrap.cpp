...
if(argc != 4){
    ...
    std::cerr << "Usage: " << executable_name << " <1 or 0 to have interaction or not> <integer number of steps> <\"fe\"=Forward-Euero, \"rk\"=RK4, \"an\"=analytical solution>" << std::endl;
...
std::string method = argv[3];           // var that changes the method used for the simulation: "eu"=Forward-Euero, "rk"=Runge Kutta 4, "an" the analytical solution
...
for (int i=0; i<=n_step; i++){
    ...
    if(!do_rel_err){
    // write position and time data on the file
